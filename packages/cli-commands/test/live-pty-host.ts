...
//
// A probe that hangs is NOT a capability verdict: the run/cleanup deadlines
// THROW an infrastructure error (the suite goes red) rather than masking a hung
// or possibly-leaked PTY behind a benign skip. Because timeouts throw, every
// RETURNED result is definitive and is memoized; a throw is naturally uncached,
// so a one-off hung probe never becomes a permanent verdict -- the next caller
// re-probes. Calls within one loaded helper-module instance share a single probe
// (Vitest may isolate test files into separate module contexts).
//
// Capability DETECTION only: each consuming suite logs its own labelled skip and
// calls ctx.skip(). (The smoke suite keeps its own copy of the probe primitives
// for now; deduplicating that final copy is a minor follow-up, not this fix.)
...
type ProbeResult = { readonly ok: true } | { readonly ok: false; readonly reason: string };
 ...
 * the only success; a non-zero exit is a definitive (skip) failure. If the run
 * deadline expires we kill the probe and, keeping the same listener, wait the
 * cleanup deadline; either timeout path THROWS (infrastructure failure), so the
 * caller goes red rather than skipping an indeterminate / possibly-leaked PTY.
 */
  ...
  return new Promise((resolveOutcome, rejectOutcome) => {
    ...
    const handleExit = (exitCode: number): void => {
      ...
      succeed(
        exitCode === 0
          ? { ok: true }
          : { ok: false, reason: `PTY probe exited ${exitCode} (expected 0)` },
      );
    };
...
/**
 * Resolve this host's live-PTY capability once and memoize the DEFINITIVE
 * result (success, or a capability skip: win32, node-pty unavailable, non-Bash
 * shell, synchronous spawn failure, non-zero probe exit). An infrastructure
 * failure (probe timeout / unconfirmed termination) THROWS and is left uncached
 * via the finally, so a one-off hung probe cannot become a permanent verdict --
 * the next caller re-probes. Concurrent callers coalesce onto the in-flight probe.
 */
