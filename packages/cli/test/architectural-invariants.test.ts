...
describe("Architectural invariants -- M G1a D99.M @viberevert/cli-commands boundaries", () => {
  ...
  it("D99.M.19b: @viberevert/cli-commands barrel does NOT export known package-internal symbols", () => {
    ...
    const FORBIDDEN_ABSENT: ReadonlyArray<string> = [
      ...
      "ExecutableProbeDeps",
      // PTY engine internals (M G4 Step 3c, D104.C/G/I) -- the engine's host
      // shell resolution + pre-spawn precondition gate + raw terminal bridge +
      // types, consumed only within the engine; internal until the guarded
      // `--pty` path is wired (Step 3d/4).
