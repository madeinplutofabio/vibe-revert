...
`--pty` needs a real TTY, the optional `node-pty` dependency, and an interactive shell where Bash 4.1 or newer can be resolved; it is unavailable via MCP. It never silently falls back: if any prerequisite is missing -- including on a stock Windows box, which resolves PowerShell -- it refuses non-zero; plain `viberevert shell` remains available. Unlike the REPL, PTY mode is not fixed-cwd: you can `cd`, and each auditable interception is recorded at its prompt-time directory. If an audit prerequisite cannot be satisfied persistently, every intercepted, auditable command stays blocked -- including `exit`; leave with Ctrl-D.

See [`docs/pty-contract.md`](https://github.com/madeinplutofabio/vibe-revert/blob/main/docs/pty-contract.md) for the full contract: the coverage/disposition distinction, bypass categories, the `[rawLine]` audit shape + hygiene, the two cwds and which one is stored (including cwd format limits, which make some legal POSIX directory names unauditable), lifecycle and terminal restoration, exit codes, and what is proven live vs. by unit tests (D104.A-O).
