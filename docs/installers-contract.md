...
- **M G1b-followup-21** — Decide whether published package surfaces should exclude `dist/*.tsbuildinfo`. **Resolved in M RH:** yes -- adapters, installers, and policies-basic aligned to the fleet convention (`tsBuildInfoFile: ./build.tsbuildinfo`, package root, gitignored, outside the packed `files` set); pack-surface verified no `.tsbuildinfo` in any of the 10 tarballs, and an architectural invariant now forbids `tsBuildInfoFile` under `dist/`.
- **M G1b-followup-22** — Decide npm dist-tag hygiene for beta packages, including the `bootstrap` tag on first-release packages and the `latest: 0.0.0` placeholder relics. **Resolved in M RH:** `latest` advanced to the current beta on all 10 publish targets (beta-phase policy documented in `docs/release-process.md`'s dist-tag discipline section); `bootstrap` tags removed; the 7 superseded `0.7.1-beta.0` versions deprecated.
