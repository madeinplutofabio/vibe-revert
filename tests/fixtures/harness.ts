...
async function runOneReceiptProducingScenario(args: {
  ...
  try {
    ...
    // Compared RAW above, redacted only for the golden. The stdout-matches-
    // persisted assertion is about this run's own bytes and must not be
    // weakened by a substitution made for cross-platform stability.
    return redactContributionDigest(result.stdout, persistedBytes, args.setup);
  } finally {
    await rm(tmpParent, { recursive: true, force: true });
  }
}
...
/**
 * What replaces the contribution digest in a golden.
 *
 * Deliberately not hex, so a stale golden regenerated by an older harness is
 * obvious on sight rather than looking like just another hash.
 */
const CONTRIBUTION_DIGEST_PLACEHOLDER = "<contribution-sha256>";

/**
 * Redact `contribution_sha256` from a rendered receipt before it becomes a
 * golden.
 *
 * WHY THIS VALUE CANNOT BE A GOLDEN BYTE. The digest is taken over
 * `contribution.json`, and that file honestly records what each platform can
 * observe about a path. On POSIX a regular file's executable bit is a real
 * boolean; on Windows the filesystem cannot report one, so the captured value
 * is `null`, meaning unknown. Measured on this fixture: flipping
 * `"executable": null` to `"executable": false` in the Windows bytes yields
 * the exact digest Linux and macOS produce, and nothing else in the file
 * differs, commit SHAs included.
 *
 * So the divergence is the artifact being truthful, not a defect, and pinning
 * the digest would demand that a Windows capture lie about what it could see.
 * The same position the project already takes on real git SHAs, which never
 * appear in a golden either (`VIBEREVERT_TEST_FIXED_SHA` puts forty zeros in
 * the report goldens instead).
 *
 * WHAT IS STILL PINNED. Everything else, byte for byte, in all three formats.
 * The digest's own correctness is not this fixture's job and is checked where
 * it matters: `loadVerifiedSessionContribution` rehashes the bytes on every
 * read, so a selective rollback that reached the point of writing a receipt
 * has already proven the digest matches.
 *
 * REDACTED BY VALUE, not by pattern. The digest is read from THIS run's
 * persisted receipt and replaced wherever it appears, so the redaction is
 * exact and needs no per-format knowledge of how each renderer labels the
 * field. A label change cannot silently stop it working.
 */
function redactContributionDigest(
  rendered: string,
  persistedBytes: string,
  setup: ReceiptFixtureSetup,
): string {
  let digest: unknown;
  try {
    digest = (JSON.parse(persistedBytes) as { contribution_sha256?: unknown }).contribution_sha256;
  } catch {
    // A receipt that will not parse is a failure the caller reports on its own
    // terms; redaction has nothing to contribute to that diagnosis.
    return rendered;
  }

  if (typeof digest !== "string") {
    // Legacy receipts carry no contribution digest at all, which is the
    // overwhelmingly common case and needs no redaction.
    if (isSelectiveInvocation(setup.rollback_invocation)) {
      throw new Error(
        "A selective receipt must carry contribution_sha256; without it the golden cannot be made platform-stable.",
      );
    }
    return rendered;
  }

  const redacted = rendered.split(digest).join(CONTRIBUTION_DIGEST_PLACEHOLDER);
  if (redacted === rendered) {
    // The renderer stopped emitting the digest. Silently accepting that would
    // leave a golden that merely happens to be stable today.
    throw new Error(
      `Rendered receipt does not contain the persisted contribution digest ${digest}, so the golden cannot be redacted. ` +
        `If the renderer intentionally dropped the field, update this redaction with it.`,
    );
  }
  return redacted;
}
